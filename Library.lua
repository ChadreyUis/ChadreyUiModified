...
do
    ...
    local Themes = {
        ["Preset"] = {
            ["Background"]       = Color3.fromRGB(18, 16, 16),
            ["Outline"]          = Color3.fromRGB(42, 36, 36),
            ["Border"]           = Color3.fromRGB(10, 7, 7),
            ["Accent"]           = Color3.fromRGB(220, 60, 60),
            ["Risky"]            = Color3.fromRGB(255, 40, 40),
            ["Light Border"]     = Color3.fromRGB(14, 10, 10),
            ["Border 2"]         = Color3.fromRGB(12, 8, 8),
            ["Text"]             = Color3.fromRGB(180, 180, 180),
            ["Section"]          = Color3.fromRGB(22, 20, 20),
            ["Element"]          = Color3.fromRGB(32, 28, 28),
            ["Hovered Element"]  = Color3.fromRGB(45, 35, 35),
    ...
    do
        ...
        Library.Font = CustomFont:New("atte", 400, "Regular", {
            Id = "atte",
            Url = "https://github.com/mental1illness/storage/raw/refs/heads/main/fonts/old-tahoma.ttf"
        })
    end
        ...
        Self:Connect("InputBegan", function(Input)
            ...
            if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                ...
                if Gui.Position.X.Scale ~= 0 or Gui.Position.Y.Scale ~= 0 then
                    ...
                    Gui.Position = UDim2.fromOffset(Gui.AbsolutePosition.X, Gui.AbsolutePosition.Y)
                end
    ...
    do
        ...
        Library.Notification = function(Self, Name, Duration, Color)
            ...
            Items["Stroke1"].Instance.Transparency = 1
            Items["AccentLiner"].Instance.BackgroundTransparency = 1
            Items["ProgressBar"].Instance.BackgroundTransparency = 1
            Items["Text"].Instance.TextTransparency = 1

            Items["Notification"].Instance.Position = UDim2.new(-1, 0, 0, 0)

            local SlideInInfo = TweenInfo.new(0.45, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out)
            local FadeInfo    = TweenInfo.new(0.35, Enum.EasingStyle.Sine, Enum.EasingDirection.Out)

            Library:Thread(function()
                Items["Notification"]:Tween({ BackgroundTransparency = 0, Position = UDim2.new(0, 0, 0, 0) }, SlideInInfo)
                Items["Stroke"]:Tween({ Transparency = 0 }, FadeInfo)
                Items["Stroke1"]:Tween({ Transparency = 0 }, FadeInfo)
                Items["AccentLiner"]:Tween({ BackgroundTransparency = 0 }, FadeInfo)
                Items["ProgressBar"]:Tween({ BackgroundTransparency = 0 }, FadeInfo)
                Items["Text"]:Tween({ TextTransparency = 0 }, FadeInfo)

                task.wait(0.45)
                local DrainInfo = TweenInfo.new(Duration, Enum.EasingStyle.Linear, Enum.EasingDirection.Out)
                Items["ProgressBar"]:Tween({ Size = UDim2.new(0, 0, 0, 1) }, DrainInfo)

                task.wait(Duration)

                local SlideOutInfo = TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.In)
                Items["Notification"]:Tween({ BackgroundTransparency = 1, Position = UDim2.new(1, 50, 0, 0) }, SlideOutInfo)
                Items["Stroke"]:Tween({ Transparency = 1 }, SlideOutInfo)
                Items["Stroke1"]:Tween({ Transparency = 1 }, SlideOutInfo)
                Items["AccentLiner"]:Tween({ BackgroundTransparency = 1 }, SlideOutInfo)
                Items["ProgressBar"]:Tween({ BackgroundTransparency = 1 }, SlideOutInfo)
                Items["Text"]:Tween({ TextTransparency = 1 }, SlideOutInfo)
        ...
        Library.Window = function(Self, Params)
            ...
            do
                Items["MainFrame"] = Library:Create("Frame", {
                    ...
                    AnchorPoint = Vector2.new(0.5, 0.5),
                    Position = UDim2.new(0.5, 0, 0.5, 0),
                    Size = UDim2.new(0, 500, 0, 590),
                    BorderSizePixel = 0,
                    BackgroundColor3 = Library.Theme["Background"]
                }):AddToTheme({ BackgroundColor3 = 'Background' })

                Items["MainFrame"]:MakeDraggable()
                Items["MainFrame"]:MakeResizeable(Vector2.new(
                ...
                -- ── TITLE: bigger (16px), shimmer-animated, two-tone ──
                Items["HeaderTitle"] = Library:Create("TextLabel", {
                    ...
                    TextSize = 16,                          -- bigger than original 12
                    TextXAlignment = Enum.TextXAlignment.Left,
                ...
                -- ── TABS ROW: fixed height, no flex-stretch ──
                Items["PagesOutline"] = Library:Create("Frame", {
                    Name = "\0",
                    Parent = Items["MainFrame"].Instance,
                    Position = UDim2.new(0, 10, 0, 24),     -- pushed down by bigger title
                    Size = UDim2.new(1, -20, 0, 22),        -- 22px tall tabs
                    BorderSizePixel = 0,
                    AutomaticSize = Enum.AutomaticSize.None,
                    BackgroundColor3 = Library.Theme["Border 2"]
                }):AddToTheme({ BackgroundColor3 = 'Border 2' })

                -- No HorizontalFlex — tabs stay their natural width
                Library:Create("UIListLayout", {
                    Name = "\0",
                ...
                Items["ContentOutline"] = Library:Create("Frame", {
                    Name = "\0",
                    Parent = Items["MainFrame"].Instance,
                    Position = UDim2.new(0, 10, 0, 47),     -- below the 22px tab row + 1px gap
            ...
            -- ── Animated title: per-character shimmer, two-tone red→gold ──
            do
                local TitleString   = "Chadreyy was here"
                local AccentColor   = Color3.fromRGB(220, 60, 60)
                local GoldColor     = Color3.fromRGB(255, 210, 60)
                local ShineColor    = Color3.fromRGB(255, 255, 255)
                local ShineWidth    = 5
                local ShimmerPhase  = 0
                local LastFrameTick = tick()

                local function BuildTitle(Phase)
                    local Out = {}
                    local Len = #TitleString
                    local ShinePos = Phase * (Len + ShineWidth)

                    for i = 1, Len do
                        local Ch = TitleString:sub(i, i)
                        if Ch == " " then
                            Out[#Out + 1] = " "
                        else
                            -- "Chadreyy" = 8 chars → accent; " was here" → gold
                            local Base  = (i <= 8) and AccentColor or GoldColor
                            local Dist  = math.abs(i - ShinePos)
                            local Alpha = math.clamp(1 - (Dist / ShineWidth), 0, 1)
                            local Final = Base:Lerp(ShineColor, Alpha * 0.55)
                            Out[#Out + 1] = string.format(
                                '<font color="rgb(%d,%d,%d)">%s</font>',
                                math.floor(Final.R * 255),
                                math.floor(Final.G * 255),
                                math.floor(Final.B * 255),
                                Ch
                            )
                        end
                    end
                ...
                local TitleConn = RunService.RenderStepped:Connect(function()
                    local Inst = Items["HeaderTitle"].Instance
                    if not Inst or not Inst.Parent then return end
                    local Now = tick()
                    ShimmerPhase = (ShimmerPhase + (Now - LastFrameTick) * 0.55) % 1
                    LastFrameTick = Now
                    Inst.Text = BuildTitle(ShimmerPhase)
                end)
                table.insert(Library.Connections, TitleConn)
            end
            ...
            function Window:Center()
                task.defer(function()
                    ...
                    local FrameSize = Items["MainFrame"].Instance.AbsoluteSize
                    Items["MainFrame"].Instance.AnchorPoint = Vector2.new(0, 0)
                    Items["MainFrame"].Instance.Position = UDim2.fromOffset(
                        math.floor((ScreenSize.X - FrameSize.X) / 2),
                        math.floor((ScreenSize.Y - FrameSize.Y) / 2)
                    )
                end)
            end
            ...
            function Window:SetTitle(Text)
                ...
                -- Title is driven by the shimmer loop; no manual set needed.
                -- Override TitleString if you want to change the displayed text:
                -- (rebuild the shimmer closure with the new string)
            end
        ...
        -- ══════════════════════════════════════════════════════════
        --  PAGE  (patched: compact fixed-width tabs, 10px text)
        -- ══════════════════════════════════════════════════════════
        Library.Page = function(Self, Params)
            ...
            local Items = {}
            do
                -- Outer tab frame: fixed compact size, no flex
                Items["Inactive"] = Library:Create("TextButton", {
                    ...
                    Text = "",
                    AutoButtonColor = false,
                    Size = UDim2.new(0, 76, 1, 0),          -- fixed 76px wide, full height of row
                    BorderSizePixel = 0,
                ...
                Items["Poophack"] = Library:Create("UIGradient", {
                    Name = "\0",
            ...
            function Page:Turn(Bool)
                ...
                if Bool then
                    ...
                    local Dummy = Instance.new("Color3Value")
                    Dummy.Value = Color3.fromRGB(255, 255, 255)

                    local Connection
                    Connection = Dummy:GetPropertyChangedSignal("Value"):Connect(function()
                        Items["Poophack"].Instance.Color = ColorSequence.new {
                            ColorSequenceKeypoint.new(0, Dummy.Value),
                            ColorSequenceKeypoint.new(1, Color3.fromRGB(255, 255, 255))
                        }
                    end)

                    local Tween = TweenService:Create(Dummy,
                        TweenInfo.new(Library.Animation.Time, Enum.EasingStyle[Library.Animation.Style],
                            Enum.EasingDirection[Library.Animation.Direction]),
                        { Value = Color3.fromRGB(172, 172, 172) }
                    )
                    Tween:Play()
                    ...
                    local Dummy = Instance.new("Color3Value")
                    Dummy.Value = Color3.fromRGB(172, 172, 172)
        ...
        Library.SubPage = function(Self, Params)
            ...
            local Items, I = {}, Page.Items
            local function C(t, p) return Library:Create(t, p) end

            I.Inactive = C("TextButton", {
                Name="\0", FontFace=Library.Font, TextSize=Library.FontSize,
                Parent=Self.Items.SubPages.Instance, TextColor3=Library.Theme.Text,
                Text=Page.Name, AutoButtonColor=false, BackgroundTransparency=1,
                Size=UDim2.new(0,0,0,15), BorderSizePixel=0,
                AutomaticSize=Enum.AutomaticSize.X
            }):AddToTheme{TextColor3='Text'}
            ...
            for _, col in ipairs({"Left","Right"}) do
                I[col.."Column"] = C("ScrollingFrame", {
                    Name="\0", Parent=I.Page.Instance,
                    ScrollBarImageColor3=Color3.fromRGB(0,0,0), Active=true,
                ...
                C("UIPadding", {
                    Name="\0", Parent=I[col.."Column"].Instance,
                    [col=="Left" and "PaddingLeft" or "PaddingRight"]=UDim.new(0,14),
                ...
                Page.ColumnsData[#Page.ColumnsData+1] = I[col.."Column"]
            end
            ...
            function Page:Turn(Bool)
                ...
                Debounce = true
                if Bool then
                    I.Inactive:ChangeItemTheme({ TextColor3 = "Accent" })
                    I.Inactive:Tween({ TextColor3 = Library.Theme.Accent })
                else
                    I.Inactive:ChangeItemTheme({ TextColor3 = "Text" })
                    I.Inactive:Tween({ TextColor3 = Library.Theme.Text })
                end
                I.Page:FadeDescendants(Bool, function()
                    Debounce = false
                    I.Page.Instance.Parent = Bool and Self.Items.Columns.Instance or Library.UnusedHolder.Instance
                end)
            end

            I.Inactive:Connect("MouseButton1Down", function()
                for _, v in ipairs(Self.Pages) do v:Turn(v == Page) end
            end)
        ...
        Library.ESPPreview = function(Self, Params)
            ...
            function Preview:AddObject(Object)
                ...
                elseif Object:IsA("Humanoid") then
                    ...
                    for _, State in ipairs(States) do
                        Clone:SetStateEnabled(State, false)
                    end
            ...
            Library:Connect(RunService.Heartbeat, function()
                if not PreviewModel or not Items["ESPPreview"].Instance.Visible then return end
        ...
        Library.InventoryViewer = function(Self, Params)
            ...
            local function RecalculateLayout()
                ...
                local DesiredWidth = math.clamp(
                    ...
                    Viewer.MinimumSize.X, Viewer.MaximumWidth
                )
                local DesiredHeight = math.clamp(
                    ContentHeight + Viewer.HeaderHeight + FramePaddingY,
                    Viewer.MinimumSize.Y, Viewer.MaximumHeight
                )
            ...
            Library:Connect(UserInputService.InputBegan, function(Input, Processed)
                if Processed or not Viewer.Visible then return end
        ...
        Library.Toggle = function(Self, Params)
            ...
            do
                ...
                Items["Text"] = Library:Create("TextLabel", {
                    ...
                    BackgroundTransparency = 1,
                    Position = UDim2.new(0, ToggleTextOffset, 0.5, -1),
                    BorderSizePixel = 0,
                    AutomaticSize = Enum.AutomaticSize.X
                }):AddToTheme({ TextColor3 = 'Inactive Text' })
